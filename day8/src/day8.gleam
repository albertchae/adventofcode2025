...
pub fn main() -> Nil {
  ...
  let shortest_1000_distances =
    distances |> shortest_distances() |> list.take(count)
  ...
  let #(ptc, ctp) = merge_circuits(shortest_1000_distances, point_to_circuit_name, circuit_name_to_points)
...
pub fn merge_circuits(
  ...
  case todo_point_pairs {
    ...
    [head, ..rest] -> {
      let #(updated_ptc, updated_ctp) = do_merge_circuit(head, point_to_circuit, circuit_to_points)
      merge_circuits(rest, updated_ptc, updated_ctp)
    }
  }
}
...
fn parse_argv() -> #(String, Int) {
  case argv.load().arguments {
    [path, count] -> #(path, result.unwrap(int.parse(count), 1000))
    _ -> {
